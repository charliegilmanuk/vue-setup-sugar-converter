...
const convert = (pattern, { destination, overwrite }) => {
  ...
  console.log(
    chalk.blue(
      `Found ${
        allFiles.filter(file => file.search(/\.vue/gi) > -1).length
      } Vue SFCs using input`
    )
  );
  ...
  if (alreadyConvertedCount) {
    console.log(
      chalk.blue(
        `Excluded ${alreadyConvertedCount} components as they appeared to have the <script setup> tag already`
      )
    );
  }

  const components = rawComponents
    .filter(x => !x.isConverted)
    .map(component => {
      const { script, template, style } = component;
      let result = script.content.trim();

      [findAndRemoveComponents, findAndRemoveEmits, findAndRemoveProps].forEach(
        fn => {
  ...
  console.log(
    chalk.bgGreen(
      `${successfulWrites.length} of ${components.length} files successfully converted!`
    )
  );
  if (failedWrites.length) {
    console.log(
      chalk.bgYellow(`${failedWrites.length} files failed`, failedWrites)
    );
  }
};
  ...
  return paths
    .map(sourcePath => {
      ...
      if (script) {
        ...
        console.log(
          chalk.yellow(
            `Skipping ${sourcePath} as there is no script tag defined.`
          )
        );
      }
    })
